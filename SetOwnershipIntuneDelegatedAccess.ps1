...
try {
    ...
    if($ExcludeMDM.IsPresent){ $Count_Params++ }
        ...
        $uri = "https://graph.microsoft.com/$graphApiVersion/$Resource`?`$filter=operatingSystem eq 'Android' and osversion eq '9.0'&$format=xml"
        Write-Warning "EAS Devices are excluded by default, please use -IncludeEAS if you want to include those devices"
        Write-Host

        }

        $outputofDevices=(Invoke-RestMethod -Uri $uri -Headers $authToken -Method Get).Value

        ######write to text

        $outputofDevices | Out-File c:\test\outputofDevices.txt
...
$Resource = "deviceManagement/managedDevices"
    ...
    try {
        ...
        $output = $output + "`n" + (Get-Date -Format g).ToString() + " No Device id specified, please provide a device id... "
        break
        ...
        if($ownerType -eq "" -or $ownerType -eq $null){
            ...
            Write-Host
            $output = $output + "`n" + (Get-Date -Format g).ToString() + " No ownerType parameter specified, please provide an ownerType. Supported value personal or company... "
            break
...
# Removed by  #      $Confirm = read-host
    ...
    Write-Error "Request to $Uri failed with HTTP Status $($ex.Response.StatusCode) $($ex.Response.StatusDescription)"
    write-host
    Write-Log " FAILED: Device Ownership NOT changed. Device ID: $($id) , Ownership: $($ownertype)"
...
function Write-Log {
    Param(
        $Message,
        $Path = "C:\test\IntuneSetDeviceOwnership.log"
    )

    function TS {Get-Date -Format g}
    "[$(TS)]$Message" | Tee-Object -FilePath $Path -Append | Write-Verbose
}


#  this path should point to the csv file that has two columns, ID and ownership
$csv=import-csv 'E:\Intune102021.csv'


foreach ($line in $csv) {Set-ManagedDevice -id $line.ID -ownertype $line.ownership}
